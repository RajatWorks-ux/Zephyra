  ...
  initialize: async () => {
    ...
    try {
      ...
      if (session?.user) {
        ...
        // Cloud key restore — AWAITED here (unlike the SIGNED_IN listener
        // below, which fires later and can stay fire-and-forget) so that by
        // the time isInitialized flips true, RootNavigator already knows
        // whether this device's keys exist. This prevents a returning user
        // from briefly flashing the wizard / New Device screen while the
        // restore is still in flight.
        try {
          const { restoreOpenRouterKeyIfNeeded } = await import('../services/secureKeyStore')
          const restored = await restoreOpenRouterKeyIfNeeded(session.user.id)
          if (restored) {
            const { useSetupStore } = await import('../store/setupStore')
            await useSetupStore.getState().initialize()
          }
        } catch (e) {
          console.warn('[AuthStore] cloud key restore failed (non-fatal):', e)
        }
      ...
      supabase.auth.onAuthStateChange(async (event, session) => {
        ...
        if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
          ...
          import('../services/secureKeyStore').then(({ restoreOpenRouterKeyIfNeeded }) => {
            restoreOpenRouterKeyIfNeeded(session.user.id).then(restored => {
              if (restored) {
