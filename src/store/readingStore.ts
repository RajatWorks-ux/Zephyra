...
//   generateFullReading was called without passing mathScore in all three call sites
//   (initialize, silentRefresh, regenerateInLanguage), causing nvidiaAI.ts to fall
//   back to `options?.mathScore ?? 65`. Fix: getDailyScore(chartData) is already
//   computed at each call site — it is now passed as mathScore so the score always
//   comes from astrologyEngine math, never from AI.
// ═══════════════════════════════════════════════════════════════════════════════
...
export const useReadingStore = create<ReadingState>((set, get) => ({
  ...
  initialize: async (userId: string, birthProfile: BirthProfile) => {
    ...
    try {
        ...
        if (localText) {
          ...
          if (parsed) {
            ...
            set({
              reading: parsed,
              currentLanguageCode: 'en-US',
              // [FIX 7] Override AI's cached score with today's fresh math score
              dailyScore: astrologyScore,
              isLoading: false,
              chaptersDone: 5,
      ...
      if (existingReading?.full_reading_text) {
        ...
        if (parsed) {
          ...
          set({
            ...
            // [FIX 7] Always use fresh math score, never the AI-cached value
            dailyScore: astrologyScore,
            isLoading: false,
            chaptersDone: 5,
      ...
      // [FIX 7] Pass math-calculated score so chunk 5 never provides daily_score_base
      ...
      if (parsed) {
        ...
        set({
          ...
          currentLanguageCode: 'en-US',
          currentLanguage: null,
          // [FIX 7] Use math score directly — parsed.daily_score_base is already mathScore
          dailyScore: astrologyScore,
          isGenerating: false,
          parallelOraclesActive: 0,
          chaptersDone: 5,
  ...
  silentRefresh: async (userId: string, birthProfile: BirthProfile) => {
    ...
    try {
      ...
      // [FIX 7] Pass fresh math score to generateFullReading
      const parsed = await generateFullReading(
        freshChartData,
      ...
      if (parsed) {
        ...
        set({
          reading: parsed,
          // [FIX 7] Use the fresh math score, not whatever AI returned in daily_score_base
  ...
  regenerateInLanguage: async (userId: string, birthProfile: BirthProfile, language: Language) => {
    ...
    const userAge = computeAge(birthProfile.birth_date)
    // [FIX 7] Compute fresh score for this regeneration
    ...
    try {
      // [FIX 7] Pass math score here too
      const parsed = await generateFullReading(
        chartData,
        (status: string, progress: number) => {
          const completedOracles = Math.max(0, Math.round((progress - 12) / 16))
      ...
      if (parsed) {
        ...
        set({
          ...
          // [FIX 7] Always use math score
          dailyScore: regenScore,
          isGenerating: false,
          isRegenerating: false,
          parallelOraclesActive: 0,
          chaptersDone: 5,
