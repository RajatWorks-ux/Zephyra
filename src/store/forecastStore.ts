...
const SYSTEM_VEDIC = `You are Zephyra, a master Vedic Jyotishi (Vedic astrologer). You speak only in pure Jyotish — sidereal zodiac, Vimshottari Dasha, classical texts (BPHS, Phaladeepika, Saravali). Every statement must name specific planets, houses, nakshatras, or Dasha periods. No generic advice. TIMING DISCIPLINE (mandatory): for any influence, transit, or Dasha period you mention, state plainly whether it is already underway, about to begin (with the actual date), or already concluded (with the actual date) relative to today. Never describe a past influence as if it were still current, and never describe a future one without its start date. Return ONLY valid JSON — no markdown, no preamble, no code fences.`
...
Generate a monthly Vedic forecast. Return ONLY this JSON:
{
  ...
  "special_alerts": [
    ...
    const specialAlerts: SpecialAlert[] = rawAlerts.map(a => ({
      type: a.type ?? 'favorable', date: a.date ?? '', title: a.title ?? '', impact: a.impact ?? '',
    }))
