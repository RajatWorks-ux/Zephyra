...
// ─────────────────────────────────────────────────────────────────────────────
// PHASE 3: Full Vedic math + AI (via OpenRouter) for every forecast tab
...
// ── AI call (via OpenRouter) with retry ──────────────────────────────────────
async function callOpenRouter(
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const res = await runQueued(apiKey, () => fetchGroqWithBackoff('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) { console.error('[Zephyra] OpenRouter error:', res.status); continue }
      const data = await res.json()
      const text = data?.choices?.[0]?.message?.content ?? ''
      if (text.length > 20) return text
    } catch (e: any) {
      console.warn('[Zephyra] OpenRouter attempt failed:', e.message)
...
Generate today's full Vedic cosmic forecast. Return exactly this JSON:
{
  ...
  "moon_sign": "current transiting Moon rashi in Sanskrit"
    ...
    const raw = await callOpenRouter(SYSTEM_VEDIC, prompt, 750)
...
Generate a 7-day Vedic forecast. The scores are already computed via Vedic mathematics — use them as-is. Return ONLY this JSON with exactly 7 entries in "days":
{
  "days": [
    ...
    const raw = await callOpenRouter(SYSTEM_VEDIC, prompt, 1400)
    const parsed = safeJSON(raw)
...
Generate a monthly Vedic forecast. Return ONLY this JSON:
{
  ...
  "special_alerts": [
    ...
    const raw = await callOpenRouter(SYSTEM_VEDIC, prompt, 850)
    const parsed = safeJSON(raw)
...
Generate a 12-month year-ahead Vedic forecast. The energy bars are already computed — use them. Return ONLY this JSON:
{
  "months": [
    ...
    const raw = await callOpenRouter(SYSTEM_VEDIC, prompt, 1600)
    const parsed = safeJSON(raw)
