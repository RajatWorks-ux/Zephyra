...
// ── PRIMARY MODELS — 3 capable models, tried first for everything ─────────────
// MODEL_1: kimi-k2.6      — primary chat + reading chunks 1, 3
// MODEL_2: mistral-small  — oracle modal + reading chunks 2, 4
// MODEL_3: mistral-medium — chunk 5 scores + seed extraction
const MODEL_1 = 'moonshotai/kimi-k2.6'
const MODEL_2 = 'mistralai/mistral-small-4-119b-2603'
const MODEL_3 = 'mistralai/mistral-medium-3.5-128b'

// ── PRIMARY KEYS (5) — one per parallel chunk ─────────────────────────────────
...
// ── LAST-RESORT KEYS (5) — from the backup file, all use mistral-small ────────
// Only reached if ALL three primary models fail on a given call.
// These are separate NVIDIA accounts so they have independent rate-limit pools.
const BACKUP_MODEL = 'mistralai/mistral-small-4-119b-2603'
const BACKUP_KEY_1 = process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_1 ?? ''
const BACKUP_KEY_2 = process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_2 ?? ''
const BACKUP_KEY_3 = process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_3 ?? ''
const BACKUP_KEY_4 = process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_4 ?? ''
const BACKUP_KEY_5 = process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_5 ?? ''

// ── FALLBACK CHAIN ────────────────────────────────────────────────────────────
// Runtime getters — evaluated at call time, NOT at module load.
// This prevents empty-string poisoning in React Native where EXPO_PUBLIC_*
// env vars may not be available yet when the module is first imported.
//
// Tier 1 — 3 primary models across the 5 main keys (tried in order):
function getPrimaryFallbackPairs(): Array<{ key: string; model: string }> {
  return [
    { key: process.env.EXPO_PUBLIC_NVIDIA_API_KEY_1 ?? '', model: MODEL_1 },
    { key: process.env.EXPO_PUBLIC_NVIDIA_API_KEY_2 ?? '', model: MODEL_2 },
    { key: process.env.EXPO_PUBLIC_NVIDIA_API_KEY_3 ?? '', model: MODEL_1 },
    { key: process.env.EXPO_PUBLIC_NVIDIA_API_KEY_4 ?? '', model: MODEL_2 },
    { key: process.env.EXPO_PUBLIC_NVIDIA_API_KEY_5 ?? '', model: MODEL_3 },
  ]
}
// Tier 2 — 5 backup keys with mistral-small (last resort, only if Tier 1 all fail):
function getBackupFallbackPairs(): Array<{ key: string; model: string }> {
  return [
    { key: process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_1 ?? '', model: BACKUP_MODEL },
    { key: process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_2 ?? '', model: BACKUP_MODEL },
    { key: process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_3 ?? '', model: BACKUP_MODEL },
    { key: process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_4 ?? '', model: BACKUP_MODEL },
    { key: process.env.EXPO_PUBLIC_NVIDIA_BACKUP_KEY_5 ?? '', model: BACKUP_MODEL },
  ]
}
...
// Priority: MODEL_1 → MODEL_2 → BACKUP_KEY_1 (mistral-small, last resort)
export async function streamAIResponse(
  ...
  const attempts = [
    { key: API_KEY_1, model: MODEL_1 },
    { key: API_KEY_2, model: MODEL_2 },
    ...getBackupFallbackPairs(),  // all 5 backup keys as last resort
  ]
    ...
    try {
      ...
      if (!response.ok) {
        if (i < attempts.length - 1) {
          console.warn(`[Zephyra] Stream ${model.split('/')[1]} HTTP ${response.status}, trying next`)
          continue
        }
      ...
      if (i < attempts.length - 1) {
        console.warn(`[Zephyra] Stream ${model.split('/')[1]} error, trying next:`, e.message)
        continue
      }
      onError(e.message)
    }
...
// ─── Non-streaming call for a specific API key + model ───────────────────────
async function getAIResponseWithKey(
  ...
  console.log(`[Zephyra] ▶ Oracle starting — ${model.split('/')[1]} key ...${apiKey.slice(-6)}`)
  ...
  try {
    ...
    clearTimeout(timer)
    if (!res.ok) {
      console.error(`[Zephyra] ✗ ${model.split('/')[1]} key ...${apiKey.slice(-6)} HTTP error: ${res.status}`)
      return ''
    }
    const data = await res.json()
    const result = data?.choices?.[0]?.message?.content || ''
    console.log(`[Zephyra] ✓ ${model.split('/')[1]} key ...${apiKey.slice(-6)} done — ${result.length} chars`)
    return result
  } catch (error: any) {
    clearTimeout(timer)
    console.error(`[Zephyra] ✗ ${model.split('/')[1]} key ...${apiKey.slice(-6)} FAILED:`, error.message)
    return ''
  }
}

// ── Two-tier automatic fallback ───────────────────────────────────────────────
// Tier 1: walk PRIMARY_FALLBACK_PAIRS (3 good models, 5 keys)
// Tier 2: if ALL of Tier 1 fails, walk BACKUP_FALLBACK_PAIRS (5 backup keys)
async function getAIResponseWithFallback(
  ...
  // Always try the intended primary first
  const result = await getAIResponseWithKey(primaryKey, primaryModel, messages, maxTokens, timeoutMs, temperature)
  if (result) return result

  // Walk remaining primary pairs
  for (const pair of getPrimaryFallbackPairs()) {
    if (pair.key === primaryKey && pair.model === primaryModel) continue
    if (!pair.key) continue
    console.warn(`[Zephyra] Primary fallback → ${pair.model.split('/')[1]}`)
    const r = await getAIResponseWithKey(pair.key, pair.model, messages, maxTokens, timeoutMs, temperature)
    if (r) return r
  }

  // All primary models exhausted — try backup keys as last resort
  console.warn('[Zephyra] ⚠ All primary models failed — switching to backup keys')
  for (const pair of getBackupFallbackPairs()) {
    if (!pair.key) continue
    console.warn(`[Zephyra] Backup fallback → key ...${pair.key.slice(-6)}`)
    const r = await getAIResponseWithKey(pair.key, pair.model, messages, maxTokens, timeoutMs, temperature)
    if (r) return r
  }

  console.error('[Zephyra] ✗ All primary AND backup models exhausted — giving up')
  return ''
}
...
export async function getAIResponse(messages: AIMessage[], temperature = 0.25): Promise<string> {
  try {
    ...
    if (!res.ok) {
      console.error(`[Zephyra] ✗ getAIResponse HTTP ${res.status}, trying fallback chain`)
      return getAIResponseWithFallback(API_KEY_2, MODEL_2, messages, 2048, 60000, temperature)
    }
    const data = await res.json()
    return data.choices?.[0]?.message?.content || ''
  } catch (e: any) {
    console.error(`[Zephyra] ✗ getAIResponse FAILED:`, e.message)
    return getAIResponseWithFallback(API_KEY_2, MODEL_2, messages, 2048, 60000, temperature)
  }
}
...
`
    : ''
  return `${langHeader}You are Zephyra, an expert Vedic Jyotishi (astrologer) with complete mastery of Jyotish Shastra (the ancient Indian science of light and time, used to read a person's destiny through planetary positions at birth). You have deeply studied and internalized every major classical Jyotish text:
...
RULE 5 — DASHA TIMING MUST BE EXACT:
...
function buildVerificationGate(): string {
  return `
MANDATORY PRE-WRITING VERIFICATION — DO THIS BEFORE GENERATING ANY OUTPUT:

Step 1 — LOCATE in the chart data above: the Lagna (ascendant sign), the active Mahadasha planet and its exact years, the active Antardasha planet, the Moon's Nakshatra, and the 3 most afflicted or powerful planets.

Step 2 — For each sentence you are about to write, ask: "Does this sentence mention at least one SPECIFIC element from this person's chart — a named Graha, a numbered Bhava, a Rashi, a Nakshatra, or a Dasha with years?" If no — DELETE the sentence before writing it.

Step 3 — Scan your output for these BANNED phrases. If found, rewrite the entire sentence:
"you may be" | "you might be" | "this can indicate" | "some people" | "those with this" | "often" used generically | "tends to" | "in many cases" | "it is possible that" | "this placement" without naming a specific planet + house | "many individuals"

Step 4 — Check: Is every Dasha mention accompanied by exact years? Is every planetary statement tied to its specific Bhava? Is every life event prediction anchored to a specific Dasha period?

If your output passes this check, proceed. If not, rewrite until it does. There is no acceptable middle ground.
...
You are a Vedic Jyotishi. Generate ONLY the following JSON fields. Base EVERYTHING on the Vedic chart data above. Reference specific Grahas (including combustion and Vargottama status), Bhavas, Nakshatras, Dashas, and Yogas by name. Mention navamshaRashi for key planets — state whether D-9 strengthens or weakens D-1 promise. If any planet is COMBUST, YOU MUST mention it. If any planet is VARGOTTAMA, highlight it prominently. Identify the Atmakaraka (highest-degree planet) in chapter_identity. No Western astrology. No generic content. Every sentence must reference this specific chart — name the planet, the house, the nakshatra, the dasha. Delete any sentence that could apply to any other person.
...
Return ONLY this JSON (start with { end with }):
{
  "compatible_signs": [exactly 3 objects — based on Vedic Rashi compatibility from THIS person's Lagna (${chartContext includes lagna}) and Moon Rashi using Koota matching and trikona/kendra relationships. Each: {"sign": "Vedic Rashi name in English e.g. Vrishabha", "percentage": number between 70 and 98}],
...
// ─── Oracle call with per-chunk retry + two-tier fallback ─────────────────────
// Retry flow:
//   attempt 0: primary key + model
//   attempt 1: same (5s delay)
//   attempt 2 (final): different primary pair
//   if all fail: walk backup keys one by one
async function getChunkWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    // On the final primary retry, switch to a different primary pair
    let useKey = apiKey
    let useModel = model
    if (attempt === retries) {
      const altPrimary = getPrimaryFallbackPairs().find(p => !(p.key === apiKey && p.model === model) && p.key)
      if (altPrimary) { useKey = altPrimary.key; useModel = altPrimary.model }
    }
    const raw = await getAIResponseWithKey(useKey, useModel, messages, maxTokens, timeoutMs, temperature)
  ...
  // All primary retries exhausted — try backup keys
  console.warn(`[Zephyra] ⚠ All primary retries failed for [${expectedKeys.join(', ')}] — trying backup keys`)
  for (const pair of getBackupFallbackPairs()) {
    if (!pair.key) continue
    console.warn(`[Zephyra] Backup chunk attempt — key ...${pair.key.slice(-6)}`)
    const raw = await getAIResponseWithKey(pair.key, pair.model, messages, maxTokens, timeoutMs, temperature)
    const parsed = parsePartialJSON(raw)
    const hasKeys = expectedKeys.every(k => !!(parsed as Record<string, unknown>)[k])
    if (hasKeys) return raw
  }
...
export async function generateFullReading(
  ...
  options?: {
    ...
    mathScore?: number  // Pre-calculated score from astrologyEngine — used for daily_score_base
  },
  ...
  const [raw1, raw2, raw3, raw4, raw5] = await Promise.all([
    ...
    // Chunk 1 — identity + past statements: MODEL_1 (kimi-k2.6)
    getChunkWithRetry(
      API_KEY_1, MODEL_1,
      [
        { role: 'system', content: systemPrompt },
    ...
    // Chunk 2 — love + career: MODEL_2 (mistral-small, different server from MODEL_1)
    getChunkWithRetry(
      API_KEY_2, MODEL_2,
      [
        { role: 'system', content: systemPrompt },
    ...
    // Chunk 3 — health + family: MODEL_1 via key 3 (parallel, different key)
    getChunkWithRetry(
      API_KEY_3, MODEL_1,
      [
        { role: 'system', content: systemPrompt },
    ...
    // Chunk 4 — purpose + now: MODEL_2 via key 4 (parallel, different key)
    getChunkWithRetry(
      API_KEY_4, MODEL_2,
      [
        { role: 'system', content: systemPrompt },
    ...
    // Chunk 5 — scores + compatibility: MODEL_3 (mistral-medium, lighter task)
    // Note: daily_score_base is NOT included here — it comes from mathScore (astrologyEngine)
    getChunkWithRetry(
      API_KEY_5, MODEL_3,
      [
        { role: 'system', content: systemPrompt },
      ...
      1500,
      ['compatible_signs', 'daily_energy_summary'],
  ...
  const merged: Partial<ParsedReading> = {
    ...
    // daily_score_base always comes from the pre-calculated math score,
    // never from AI — prevents hallucinated scores and removes the need for
    // AI to generate it (one less thing for chunk 5 to get right)
  ...
  const required: (keyof ParsedReading)[] = [
    ...
    // daily_score_base intentionally omitted — always set from mathScore above
  ]
  ...
  for (const k of missing) {
    ...
    if (typeof merged[key] === 'undefined' || merged[key] === null) {
      if (k === 'past_statements' || k === 'present_statements') {
...
export async function getChartInsight(
  ...
  // Language instruction is inside the system prompt (not appended after),
  // so it is processed with highest priority
  const langSection = languageInstruction
    ? `LANGUAGE INSTRUCTION (HIGHEST PRIORITY): ${languageInstruction} Write all three sections entirely in this language. Section headers must also be translated.\n\n`
    : ''

  const systemPrompt = `${langSection}You are Zephyra, a master Jyotishi (Vedic astrologer) with 40 years of experience in Brihat Parashara Hora Shastra, Phaladeepika, and Brihat Jataka. You speak truth without sugarcoating — if a placement causes suffering you name it clearly, directly, and with compassion. If a planet is COMBUST, VARGOTTAMA, or placed in Navamsha differently than D-1, YOU MUST MENTION IT — these are critical for accuracy.
...
INTERPRETATION
Write one substantial paragraph (6–8 sentences) explaining what this specific placement, period, or nakshatra truly means for this person. Reference the specific planet, house, sign, and nakshatra involved. If the planet is combust — explain what that means for this person. If Vargottama — celebrate it. Check navamsha sign and comment on whether the planet delivers its promise. Explain every Sanskrit term the first time. Be completely honest — if it is difficult say so.

EFFECTS ON YOUR LIFE
Write one substantial paragraph (6–8 sentences) describing the real-life domains this placement touches — career, relationships, health, finances, family, spirituality. Be specific to this person's chart context. Name actual life events or patterns this combination typically creates. Tie timing to the current Dasha period.

REMEDIES & SOLUTIONS
Write one paragraph listing 2–3 concrete Vedic remedies. Always include: one mantra (with the actual Sanskrit text), one gemstone or substitution remedy with specific wearing instructions, and one behavioral or donation-based remedy. Be specific — not generic. Close with: "These remedies reduce friction — they do not erase the karma. The pattern will still ask to be lived through."`
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return getChartInsightFallback(topic, contextData, systemPrompt)
    }
...
// Fallback oracle using MODEL_1 if MODEL_2 fails, then backup key if MODEL_1 also fails
async function getChartInsightFallback(
  topic: string,
  contextData: string,
  systemPrompt: string,
): Promise<string> {
  console.log(`[Zephyra] ▶ Chart insight fallback for topic: "${topic}"`)
  ...
  try {
    ...
    console.warn(`[Zephyra] Chart insight MODEL_1 fallback failed (${res.status}), trying backup key`)
  } catch (e: any) {
    console.warn(`[Zephyra] Chart insight MODEL_1 fallback error:`, e.message)
  }

  // Last resort — walk ALL backup keys (runtime getter so env vars are resolved now)
  const backupPairs = getBackupFallbackPairs()
  for (const pair of backupPairs) {
    if (!pair.key) continue
    console.warn(`[Zephyra] Chart insight backup key ...${pair.key.slice(-6)}`)
    try {
      const res = await fetch(`${NVIDIA_BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${pair.key}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: pair.model,
          messages: [
            { role: 'system', content: systemPrompt },
      ...
      if (!res.ok) {
        console.warn(`[Zephyra] Chart insight backup key ...${pair.key.slice(-6)} HTTP ${res.status}`)
        continue
      }
      const data = await res.json()
      const result = data?.choices?.[0]?.message?.content ?? ''
      ...
      console.warn(`[Zephyra] Chart insight backup key ...${pair.key.slice(-6)} FAILED:`, e.message)
    }
  }

  console.error('[Zephyra] ✗ Chart insight all backup keys exhausted')
  return ''
}
