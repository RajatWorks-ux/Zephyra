...
async function getAIResponseWithKey(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[Zephyra] ✗ GROQ key ...${apiKey.slice(-6)} HTTP error: ${res.status}`)
      return ''
    }
...
export async function generateFullReading(
  ...
  // Small helper: delays the START of a chunk request by a fixed offset
  // before it even reaches the queue. This doesn't change request content,
  // ordering guarantees, or max_tokens — it just gives the per-key token
  // tracker in groqQueue a chance to register each chunk's usage before the
  // next one on the same key evaluates its own headroom, instead of three
  // chunks on key1 all checking "tokens used so far" in the same instant.
  const delayStart = <T,>(ms: number, fn: () => Promise<T>): Promise<T> =>
    ms === 0 ? fn() : new Promise<void>(resolve => setTimeout(resolve, ms)).then(fn)

  // Chunks 1,3,5 use key1; chunks 2,4 use key2 (parallel, separate rate limit
  // pools). Within each key, chunks are given a small staggered start so they
  // don't all land in the exact same TPM-window instant.
  const [raw1, raw2, raw3, raw4, raw5] = await Promise.all([
    delayStart(0, () => getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
    ...
    delayStart(0, () => getChunkWithRetry(false, [
      { role: 'system', content: systemPrompt },
    ...
    delayStart(600, () => getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
    ...
    delayStart(600, () => getChunkWithRetry(false, [
      { role: 'system', content: systemPrompt },
    ...
    delayStart(1200, () => getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
