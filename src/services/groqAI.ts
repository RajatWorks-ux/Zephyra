...
//   nvidia/nemotron-3-super-120b-a12b, called via the OpenAI-compatible
...
// Nemotron-3-Super is a hybrid reasoning model — it can emit a separate
...
async function getAIResponseWithKey(
  ...
  // immediately, but chunks 2-5 sit waiting in line while chunk 1's (slow,
  // thinking-enabled) response streams back, which alone can take well over
  // a minute. If the abort timer were started here, outside the queue, every
  // queued chunk's clock would already be ticking down during that wait —
  // so by the time chunk 4 or 5 finally got its turn, its timeout could have
  // nearly or fully expired already, aborting it within moments of actually
  // starting. That is exactly what produced the "FAILED: Aborted" errors
  // immediately after chunk 1's success. Creating the AbortController AND
  // its setTimeout inside the runQueued callback means the full timeoutMs
  // budget is reserved for the network call itself, after this chunk's
  // queue wait is over — not shared with (or consumed by) time spent
