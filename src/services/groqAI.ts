...
//   Same model (Llama 3.3 70B Instruct), routed via OpenRouter instead of
//   calling Groq directly. OpenRouter's free tier is request-count limited
//   (RPM/RPD), not the small per-key TPM ceiling Groq enforced, so the old
//   2-key split-the-load workaround is no longer needed — one key is enough.
...
async function getAIResponseWithKey(
  ...
  console.log(`[Zephyra] ▶ OpenRouter starting — key ...${apiKey.slice(-6)}`)
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), timeoutMs)
  try {
    // Queued per-key so overlapping calls on the same key don't burst the
    // request-rate limit. With a single OpenRouter key there's no second
    // pool to spread load across, but the queue still smooths out bursts
    // from parallel chunk dispatch.
    ...
    const res = await runQueued(apiKey, () => fetchGroqWithBackoff(`${OPENROUTER_BASE_URL}/chat/completions`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) {
        ...
        console.warn(`[Zephyra] OpenRouter 429 — waiting ${waitMs / 1000}s`)
        await new Promise(resolve => setTimeout(resolve, waitMs))
      }
      let errorBody = ''
      try { errorBody = await res.text() } catch {}
      console.error(`[Zephyra] ✗ OpenRouter key ...${apiKey.slice(-6)} HTTP error: ${res.status} — ${errorBody.slice(0, 300)}`)
      return ''
    }
    const data = await res.json()
    const result = data?.choices?.[0]?.message?.content || ''
    console.log(`[Zephyra] ✓ OpenRouter key ...${apiKey.slice(-6)} done — ${result.length} chars`)
    return result
  } catch (error: any) {
    clearTimeout(timer)
    console.error(`[Zephyra] ✗ OpenRouter key ...${apiKey.slice(-6)} FAILED:`, error.message)
    return ''
  }
}
...
export async function streamAIResponse(
  ...
  try {
    const doFetch = () => runQueued(apiKey, () => fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
...
async function getChunkWithRetry(
  ...
  if (!apiKey) {
    console.error(`[Zephyra] ✗ No OpenRouter key set — cannot generate chunk [${expectedKeys.join(', ')}]`)
    return ''
  }
...
// ─── MAIN: Generate full reading via 5 parallel OpenRouter calls ─────────────
export async function generateFullReading(
  ...
  const apiKey = await getApiKey()
  if (!apiKey) {
    console.error('[Zephyra] No OpenRouter key set — cannot generate reading')
    return null
  }
  ...
  // All 5 chunks share the single OpenRouter key, queued via runQueued so
