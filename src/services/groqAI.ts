...
// Infrastructure changed: NVIDIA hardcoded keys → GROQ keys from SecureStore
...
import { getKey, KEY_GROQ_1, KEY_GROQ_2 } from './secureKeyStore'
import { runQueued, fetchGroqWithBackoff, estimateRequestTokens } from './groqQueue'

const GROQ_BASE_URL = 'https://api.groq.com/openai/v1'
const GROQ_MODEL = 'llama-3.3-70b-versatile'

// ── Read keys from SecureStore at call time (never at module load) ─────────────
async function getKeys(): Promise<{ key1: string; key2: string }> {
...
// ── Non-streaming call with a specific key ─────────────────────────────────────
async function getAIResponseWithKey(
  ...
  console.log(`[Zephyra] ▶ GROQ starting — key ...${apiKey.slice(-6)}`)
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), timeoutMs)
  try {
    // Queued per-key: if another call on this SAME key is already in flight,
    // wait for it. key1 and key2 still run fully in parallel with each
    // other. Combined with fetchGroqWithBackoff, this is what actually
    // fixes the 413 errors — Groq's free tier counts tokens-per-minute PER
    // KEY, and firing several chunks on the same key at once blows through
    // it. We pass a token estimate so the queue can proactively space
    // requests out BEFORE hitting the real limit, not just react after.
    ...
    const tokenEstimate = estimateRequestTokens(requestBody, maxTokens)
    const res = await runQueued(apiKey, () => fetchGroqWithBackoff(`${GROQ_BASE_URL}/chat/completions`, {
      method: 'POST',
    ...
    clearTimeout(timer)
    if (!res.ok) {
      // Read the real error body — Groq's 413/429 responses include the
      // ACTUAL limit and requested token counts (e.g. "Limit 12000, Requested
      // 31912"), which is far more reliable than any local estimate.
      let errorBody = ''
      ...
      if (res.status === 429) {
        ...
        console.warn(`[Zephyra] GROQ 429 — waiting ${waitMs / 1000}s`)
        await new Promise(resolve => setTimeout(resolve, waitMs))
      }
      console.error(`[Zephyra] ✗ GROQ key ...${apiKey.slice(-6)} HTTP error: ${res.status} — ${errorBody.slice(0, 300)}`)
      return ''
    }
    const data = await res.json()
    const result = data?.choices?.[0]?.message?.content || ''
    console.log(`[Zephyra] ✓ GROQ key ...${apiKey.slice(-6)} done — ${result.length} chars`)
    return result
  } catch (error: any) {
    clearTimeout(timer)
    console.error(`[Zephyra] ✗ GROQ key ...${apiKey.slice(-6)} FAILED:`, error.message)
    return ''
  }
}

// ── Two-key fallback: try key1, if fails try key2, if both fail return error ───
async function getAIResponseWithFallback(
  ...
  const { key1, key2 } = await getKeys()

  if (!key1 && !key2) {
    return { result: '', error: 'API_KEYS_NOT_SET' }
  }

  if (key1) {
    const r = await getAIResponseWithKey(key1, messages, maxTokens, timeoutMs, temperature)
    if (r) return { result: r }
  }

  if (key2) {
    console.warn('[Zephyra] Key 1 failed — trying key 2')
    const r = await getAIResponseWithKey(key2, messages, maxTokens, timeoutMs, temperature)
    if (r) return { result: r }
  }

  return { result: '', error: 'API_KEYS_EXHAUSTED' }
}
...
export async function streamAIResponse(
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i]
    try {
      const doFetch = () => runQueued(key, () => fetch(`${GROQ_BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: {
      ...
      // 413/429 from Groq on the free tier almost always means "you're over
      // the tokens-per-minute budget for this key right now" — back off
      // once and retry the SAME key before giving up on it.
      if (response.status === 413 || response.status === 429) {
        console.warn(`[Zephyra] Stream HTTP ${response.status} on key ...${key.slice(-6)} — backing off and retrying once`)
      ...
      if (!response.ok) {
        if (i < keys.length - 1) {
          console.warn(`[Zephyra] Stream HTTP ${response.status}, trying next key`)
          continue
        }
      ...
      if (i < keys.length - 1) {
        console.warn(`[Zephyra] Stream error, trying next key:`, e.message)
        continue
      }
      onError(e.message)
    }
...
// ── Per-chunk retry: key1 → key2 → error ──────────────────────────────────────
async function getChunkWithRetry(
  ...
  const primaryKey = preferKey1 ? key1 : key2
  const fallbackKey = preferKey1 ? key2 : key1

  for (let attempt = 0; attempt <= retries; attempt++) {
    const useKey = attempt < retries ? primaryKey : fallbackKey
    if (!useKey) continue
    const raw = await getAIResponseWithKey(useKey, messages, maxTokens, timeoutMs, temperature)
  ...
  // Try other key as last resort — give it a short breathing window first,
  // since it's likely also been busy with its own chunks on the same
  // generation run and may be close to its own TPM ceiling.
  if (fallbackKey) {
    await new Promise(resolve => setTimeout(resolve, 3000))
    const raw = await getAIResponseWithKey(fallbackKey, messages, maxTokens, timeoutMs, temperature)
    const parsed = parsePartialJSON(raw)
    const hasKeys = expectedKeys.every(k => !!(parsed as Record<string, unknown>)[k])
    if (hasKeys) return raw
  }
...
export function buildRelationshipSystemPrompt(
  ...
  const langHeader = language && language.code !== 'en-US'
    ...
    : ''
  return `${langHeader}You are Zephyra, a master Vedic Jyotishi specializing in synastry and relationship compatibility. You analyze two birth charts using Ashta Koota matching, house overlays, and Dasha timing. The relationship type is: ${relationshipType}. Apply the same iron law of specificity — every statement must reference specific planets, houses, nakshatras, or dashas from both charts. No generic compatibility statements.`
}
...
Return ONLY this JSON (start with { end with }):
{
  ...
  try {
    const { key1 } = await getKeys()
    if (!key1) return null
    const raw = await getAIResponseWithKey(key1, [
...
// ─── MAIN: Generate full reading via 5 parallel GROQ calls ───────────────────
export async function generateFullReading(
  ...
  const { key1, key2 } = await getKeys()
  if (!key1 && !key2) {
    console.error('[Zephyra] No GROQ keys set — cannot generate reading')
    return null
  }
  ...
  // Chunks 1,3,5 use key1; chunks 2,4 use key2 — these are separate rate
  // limit pools so the two KEYS still run in parallel with each other.
  // BUT chunks sharing the SAME key now run fully sequentially (one
  // completes before the next starts) instead of via Promise.all. Each
  // individual chunk request is large relative to the free-tier per-minute
  // budget, so giving every chunk its own fresh window — rather than
  // stacking 2-3 of them into the same window — is what actually reduces
  // 413s here, not just spacing the start times slightly apart.
  async function runKey1Chunks() {
    const r1 = await getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: buildChunk1Prompt(chartContext, age, seed, language) },
    ], 6000, ['past_statements', 'chapter_identity'])
    onChunkDone(0)

    const r3 = await getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: buildChunk3Prompt(chartContext, age, seed, language) },
    ], 8192, ['chapter_health', 'chapter_family'])
    onChunkDone(2)

    const r5 = await getChunkWithRetry(true, [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: buildChunk5Prompt(chartContext, language) },
    ], 1500, ['compatible_signs', 'daily_energy_summary'])
    onChunkDone(4)

    return [r1, r3, r5] as const
  }

  async function runKey2Chunks() {
    const r2 = await getChunkWithRetry(false, [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: buildChunk2Prompt(chartContext, age, seed, language) },
    ], 8192, ['chapter_love', 'chapter_career'])
    onChunkDone(1)

    const r4 = await getChunkWithRetry(false, [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: buildChunk4Prompt(chartContext, age, seed, language) },
    ], 6000, ['chapter_purpose', 'chapter_now'])
    onChunkDone(3)

    return [r2, r4] as const
  }

  const [[raw1, raw3, raw5], [raw2, raw4]] = await Promise.all([
    runKey1Chunks(),
    runKey2Chunks(),
  ])
...
export async function getChartInsight(
  ...
  const systemPrompt = `${langSection}You are Zephyra, a master Jyotishi with 40 years of experience. Structure your response as exactly three labeled sections — INTERPRETATION, EFFECTS ON YOUR LIFE, REMEDIES & SOLUTIONS — with no markdown, no asterisks, plain flowing text only. Each section one substantial paragraph (6-8 sentences). Be completely honest and chart-specific. End REMEDIES with: "These remedies reduce friction — they do not erase the karma."`
