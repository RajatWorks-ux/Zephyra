...
export async function streamAIResponse(
  ...
  try {
    const doFetch = () => runQueued(apiKey, () => fetch(`${NVIDIA_BASE_URL}/chat/completions`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      ...
      body: buildNvidiaBody(messages, 800, temperature, true),
    }))

    let response = await doFetch()

    // 429 means we're over the request-rate limit right now — back off once
    // and retry the same key before giving up.
...
export async function getChartInsight(
  ...
  const todayFull = new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
  const systemPrompt = `${langSection}You are Zephyra, a master Jyotishi with 40 years of experience. Today's date is ${todayFull} — use this as your reference point for everything below. ${TIMING_DISCIPLINE} Structure your response as exactly three labeled sections — INTERPRETATION, EFFECTS ON YOUR LIFE, REMEDIES & SOLUTIONS — with no markdown, no asterisks, plain flowing text only. Each section one substantial paragraph (6-8 sentences). Be completely honest and chart-specific. End REMEDIES with: "These remedies reduce friction — they do not erase the karma."`
...
export async function getForecastOracleInsight(
  ...
  const forecastSystemPrompt = `${langSection}You are Zephyra, a master Vedic Jyotishi giving a fast, practical, deeply detailed action briefing for ${periodLabel} — not a general reading. Today's date is ${todayFull}. Be extremely concrete: name the exact planet, house, nakshatra, or Dasha period behind every single item. No vague filler like "be cautious" or "good things are coming" without naming why. ${TIMING_DISCIPLINE} Specifically: "best_window" must name an actual date or date range within this period, not just "later this week." "watch_for" must say whether the caution is already underway, about to begin (with a date), or wrapping up (with a date). Return ONLY valid JSON, no markdown, no preamble, no code fences, matching exactly this shape:
{
  "verdict": "1-2 sentence direct verdict on the overall energy of this period, naming the dominant planet/Dasha and whether it is already active, just beginning, or ending — with the relevant date",
  "do_now": ["5-7 concrete, specific actions to take, each grounded in a named planet/house/nakshatra"],
  "avoid_now": ["5-7 concrete, specific things to avoid, each grounded in a named planet/house/nakshatra"],
  "best_window": "1-2 sentences naming the single best specific date or date range within this period and why, by planet/hora",
  "watch_for": "1-2 sentences on the single most important caution for this period, naming the specific planetary cause and its date — already happening, starting on a date, or ending on a date"
}`
