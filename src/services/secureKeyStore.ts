...
//
// PHASE 4 NOTE: The text-AI key (constant name KEY_OPENROUTER, kept as-is on
// purpose so every importing file — groqAI.ts, chatStore.ts, forecastStore.ts,
...
// model nvidia/nemotron-3-super-120b-a12b. The NVIDIA TTS key (KEY_NVIDIA_TTS)
...
// ── Test the NVIDIA NIM text key validity ──────────────────────────────────────
// Function name kept as testOpenRouterKey (not renamed) so the two onboarding
// screens (ApiSetupScreen.tsx, NewDeviceApiSetupScreen.tsx) don't need their
// import names changed — only what the key is tested against changes.
...
// Use this instead of setKey() for the NVIDIA text key so it survives reinstall.
...
// ── Restore NVIDIA text key from cloud if local store is empty ────────────────
...
export async function restoreOpenRouterKeyIfNeeded(userId: string): Promise<boolean> {
  ...
  if (existing && existing.startsWith('nvapi-')) return false // already have a key
