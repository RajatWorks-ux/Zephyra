...
// ─── NEW: Full Moon sign + nakshatra + pada for a specific date ───────────────
// Used by the Forecast "Today" tab's Moon section, which needs all three
// (sign, nakshatra, pada) rather than just the nakshatra name above.
export interface MoonPositionForDate {
  rashi: string
  nakshatra: string
  pada: number
  nakshatraLord: string
}
export function getMoonSignAndPadaForDate(date: Date): MoonPositionForDate {
  const jd = julianDay(date.getFullYear(), date.getMonth() + 1, date.getDate(), 12)
  const moonSid = sidereal(moonLongitude(jd), date.getFullYear())
  const nak = getNakshatra(moonSid)
  const rashiIdx = Math.floor(moonSid / 30) % 12
  return { rashi: RASHIS[rashiIdx], nakshatra: nak.name, pada: nak.pada, nakshatraLord: nak.lord }
}

// ─── NEW: Moon phase (tithi-based) name + waxing/waning + paksha + icon key ───
// 8 classical phases derived from the same moonAge calculation already used
// in computeDailyScoreForDate, so the phase shown always matches the score
// math rather than drifting from a separately-computed value.
export interface MoonPhaseInfo {
  name: string
  icon: 'new' | 'waxing-crescent' | 'first-quarter' | 'waxing-gibbous' | 'full' | 'waning-gibbous' | 'last-quarter' | 'waning-crescent'
  paksha: 'Shukla Paksha' | 'Krishna Paksha'
  tithi: number // 1-15 within the current paksha
}
export function getMoonPhaseForDate(date: Date): MoonPhaseInfo {
  ...
  const lunarCycle = 29.53
  const moonAge = ((jd - knownNewMoon) % lunarCycle + lunarCycle) % lunarCycle
  // 8 equal octants of the 29.53-day cycle
  const octant = Math.floor((moonAge / lunarCycle) * 8) % 8
  const phases: { name: string; icon: MoonPhaseInfo['icon'] }[] = [
    { name: 'New Moon (Amavasya)', icon: 'new' },
    { name: 'Waxing Crescent', icon: 'waxing-crescent' },
    { name: 'First Quarter', icon: 'first-quarter' },
    { name: 'Waxing Gibbous', icon: 'waxing-gibbous' },
    { name: 'Full Moon (Purnima)', icon: 'full' },
    { name: 'Waning Gibbous', icon: 'waning-gibbous' },
    { name: 'Last Quarter', icon: 'last-quarter' },
    { name: 'Waning Crescent', icon: 'waning-crescent' },
  ]
  const paksha: MoonPhaseInfo['paksha'] = moonAge < 14.77 ? 'Shukla Paksha' : 'Krishna Paksha'
  const tithi = Math.max(1, Math.min(15, Math.floor((moonAge % 14.77) / (14.77 / 15)) + 1))
  return { ...phases[octant], paksha, tithi }
}
...
// ─── NEW: Pick the N best-scoring specific dates within a given month ────────
// Used by the Forecast "Month" tab to turn the reading's best_months_love /
// best_months_money (which are just month numbers, e.g. [3, 7, 11]) into
// actual calendar dates within THIS month, using the same Vedic-math daily
// score already computed for the calendar grid — so "best day for love" is
// grounded in real planetary math, not a guess.
export function getBestDatesInMonth(
  natalChart: VedicChart,
  year: number,
  month0: number, // 0-indexed, matches Date.getMonth()
  count: number = 4,
): { date: string; score: number }[] {
  const daysInMonth = new Date(year, month0 + 1, 0).getDate()
  const scored = Array.from({ length: daysInMonth }, (_, i) => {
    const d = new Date(year, month0, i + 1)
    return { date: d.toISOString().split('T')[0], score: computeDailyScoreForDate(natalChart, d) }
  })
