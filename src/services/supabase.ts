...
export async function restoreApiKeysFromCloud(
  ...
  try {
    ...
    // Support both legacy OpenRouter keys (sk-or-) and current NVIDIA keys (nvapi-)
    if (!key || (!key.startsWith('nvapi-') && !key.startsWith('sk-or-'))) return null
    return { key }
  } catch (e) {
...
export async function saveRelationshipProfile(userId: string, data: any) {
  return supabase
    .from('relationship_profiles')
    .insert({ user_id: userId, ...data, created_at: new Date().toISOString(), updated_at: new Date().toISOString() })
    .select()
    .single()
}

export async function updateRelationshipProfile(profileId: string, data: any) {
  return supabase
    .from('relationship_profiles')
...
export async function deleteRelationshipProfile(profileId: string) {
  return supabase.from('relationship_profiles').delete().eq('id', profileId)
}

// ════════════════════════════════════════════════════════════════════════════
// COMPATIBILITY RESULTS  (Phase 3)
// ════════════════════════════════════════════════════════════════════════════

export async function getCompatibilityResult(userId: string, person2Id: string) {
  return supabase
...
export async function saveCompatibilityResult(userId: string, person2Id: string, data: any) {
  // Delete old result first, then insert fresh (replace pattern)
  await supabase
    .from('compatibility_results')
    .delete()
    .eq('user_id', userId)
  ...
  return supabase
    .from('compatibility_results')
    .insert({ user_id: userId, person2_id: person2Id, ...data, created_at: new Date().toISOString() })
    .select()
    .single()
}

// ════════════════════════════════════════════════════════════════════════════
// READING HISTORY LOG  (Phase 3)
// ════════════════════════════════════════════════════════════════════════════

export async function getReadingHistory(userId: string, limit = 20) {
  return supabase
    .from('reading_history_log')
    .select('*')
    .eq('user_id', userId)
...
export async function appendReadingHistoryLog(userId: string, entry: {
  trigger: string
  ...
  return supabase
    .from('reading_history_log')
    .insert({ user_id: userId, generated_at: new Date().toISOString(), ...entry })
}
