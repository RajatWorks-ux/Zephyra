...
const KNOB_R   = 11;
// Knob sits just outside OUTER_R, fully inside the SVG canvas
const KNOB_DIST = OUTER_R - KNOB_R - 2;   // knob CENTER sits here, inside ring
            ...
            <Svg width={SIZE} height={SIZE}>
              ...
              <G rotation={wheelRot} origin={`${CX},${CY}`}>
                ...
                {NAKSHATRAS.map((nak, i) => {
                  ...
                  let strokeW     = 0.8;
                  if (isArm)        { fillAlpha = 0.88; strokeColor = '#FF3333'; strokeW = 2.5; }
                  else if (isBirth) { fillAlpha = 0.75; strokeColor = nak.color;  strokeW = 2.0; }
                  ...
                  return (
                    <G key={nak.name}>
                      {isArm && (
                        <Path d={mainPath} fill="none" stroke="#FF3333" strokeWidth={16} opacity={0.18} />
                      )}
                      {isBirth && !isArm && (
                        <Path d={mainPath} fill="none" stroke={nak.color} strokeWidth={14} opacity={0.14} />
                      )}

                      <Path
                        d={mainPath}
                        fill={hexToRgba(isArm ? '#FF3333' : nak.color, fillAlpha)}
                        stroke={strokeColor}
                        strokeWidth={strokeW}
                      />
                      ...
                      <SvgText
                        ...
                        fill={
                          isArm    ? 'rgba(255,90,90,1)' :
                          isBirth  ? hexToRgba(nak.color, 1.0) :
                ...
                {/* ── Oracle arm (rotates WITH wheel so knob stays over its nak) ── */}
                {/* We counter-rotate by -wheelRot so arm stays in screen-space */}
                <G rotation={armAngle - wheelRot} origin={`${CX},${CY}`}>
                  {/* Dashed stick */}
                  <Line
                    x1={CX} y1={CY - CENTER_R - 6}
                    x2={CX} y2={CY - KNOB_DIST + KNOB_R}
                    stroke="rgba(201,168,76,0.65)"
                    strokeWidth={1.5}
                    strokeDasharray="5 3"
                  />
                  {/* Knob outer glow ring */}
                  <Circle
                  ...
                  <Circle
                    ...
                    opacity={0.95}
                  />
                  {/* Small arrow tip pointing at segment */}
                  <Polygon
                    points={`${CX},${CY - KNOB_DIST - KNOB_R - 4} ${CX - 3.5},${CY - KNOB_DIST - KNOB_R + 3} ${CX + 3.5},${CY - KNOB_DIST - KNOB_R + 3}`}
                    fill="#C9A84C"
                    opacity={0.8}
                  />
                </G>
