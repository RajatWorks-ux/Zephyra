...
export function SplashScreen({ navigation }: Props) {
  ...
  // Divider line (animated via scaleX, not width, so it can run on the
  // native driver like everything else — animating `width` with
  // useNativeDriver:false while neighbouring animations use
  // useNativeDriver:true on values that share a render tree is what was
  // throwing "Style property 'width' is not supported by native animated
  // module" and the follow-up "JS driven animation on a node moved to
  // native" crash.)
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Animated.parallel([
        ...
        Animated.timing(lineScale,   { toValue: 1, duration: 600, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      ]).start()
    }, lineDelay)
    ...
    <Animated.View style={[styles.root, { opacity: screenOpacity }]}>
      ...
      <View style={styles.center}>
        ...
        <View style={styles.decorRow}>
          ...
          <Animated.View style={[styles.decorLine, { opacity: lineOpacity, transform: [{ scaleX: lineScale }] }]} />
