// src/screens/main/ForecastScreen.tsx — PHASE 4: old design restored as base
// ─────────────────────────────────────────────────────────────────────────────
// What changed from the two versions you had:
//  • Base layout/visuals are the OLD design (the one you liked).
//  • Tab-bar overlap is fixed everywhere using real safe-area insets instead
//    of a flat guess — nothing sits hidden behind the floating tab bar now,
//    on Old OR New, on any device.
//  • Monthly Energy Calendar keeps the NEW design: weekday header row,
//    correct day-of-week offset alignment, and tap-a-day detail view.
//  • "Ask Oracle" now only appears on Week / Month / Year — NOT on Today —
//    and its navigation bug is fixed (it was pointing at a route called
//    "Chat" that doesn't exist; the real tab route is "ChatTab").
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Ask Oracle button — only used on Week / Month / Year tabs ─────────────────
function OracleButton({ context }: { context: string }) {
  const navigation = useNavigation<any>()
  return (
    <TouchableOpacity
      style={styles.oracleBtn}
      // FIX: this used to navigate('Chat', ...) — there is no route called
      // "Chat" in the tab navigator, only "ChatTab", so this silently did
      // nothing before. Also wired up to actually land the prefill text.
      onPress={() => navigation.navigate('ChatTab', { prefill: context })}
      activeOpacity={0.8}
    >
      <LinearGradient
...
// ── Today Tab ─────────────────────────────────────────────────────────────────
function TodayTab({ userId, chartData, bottomPad }: { userId: string; chartData: any; bottomPad: number }) {
  ...
  return (
    <ScrollView contentContainerStyle={[styles.tabContent, { paddingBottom: bottomPad }]} showsVerticalScrollIndicator={false}>
      ...
      {/* No Ask Oracle button here on purpose — Today/Day tab is the one
          tab where it should NOT appear. */}
    </ScrollView>
  )
}

// ── Week Tab ──────────────────────────────────────────────────────────────────
function WeekTab({ userId, chartData, bottomPad }: { userId: string; chartData: any; bottomPad: number }) {
...
// ── Month Tab ─────────────────────────────────────────────────────────────────
// Calendar grid keeps the NEW design: weekday header row + correct offset
// alignment for the 1st of the month + tap-a-day detail panel.
function MonthTab({ userId, chartData, bottomPad }: { userId: string; chartData: any; bottomPad: number }) {
  ...
  // Compute starting weekday offset (0=Sun, 1=Mon ... 6=Sat) so day 1 lines
  ...
  return (
    <ScrollView contentContainerStyle={[styles.tabContent, { paddingBottom: bottomPad }]} showsVerticalScrollIndicator={false}>
      ...
      <BlurView intensity={12} tint="dark" style={styles.card}>
        ...
        {/* Grid with prefix offset so day 1 lands under the right weekday */}
...
// ── Year Tab ──────────────────────────────────────────────────────────────────
function YearTab({ userId, chartData, bottomPad }: { userId: string; chartData: any; bottomPad: number }) {
...
export function ForecastScreen() {
  ...
  // FIX: real clearance for the floating tab bar instead of a flat guess —
  // this is what was letting content sit hidden underneath it.
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
          {!chartData ? (
            ...
            <>
              {activeTab === 'today' && <TodayTab userId={userId} chartData={chartData} bottomPad={bottomPad} />}
              {activeTab === 'week' && <WeekTab userId={userId} chartData={chartData} bottomPad={bottomPad} />}
              {activeTab === 'month' && <MonthTab userId={userId} chartData={chartData} bottomPad={bottomPad} />}
              {activeTab === 'year' && <YearTab userId={userId} chartData={chartData} bottomPad={bottomPad} />}
            </>
          )}
        </Animated.View>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  // Month heatmap (new design — weekday headers + offset alignment)
