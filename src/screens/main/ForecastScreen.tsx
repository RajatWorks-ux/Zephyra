...
function LoadingState({ label }: { label: string }) {
  // A static spinner can look frozen if the request genuinely takes a
  // while (the free-tier NVIDIA endpoint can take well over 15-20 seconds
  // under load). Cycling through a few staged, in-character messages gives
  // ongoing reassurance that something is still happening, without needing
  // real progress data from the request itself.
  const stages = [
    label,
    'Consulting the planetary positions...',
    'Cross-referencing your Dasha periods...',
    'The oracle is taking extra care with this one...',
  ]
  ...
  return (
    <View style={styles.centerState}>
      ...
      <Text style={styles.loadingLabel}>{stages[stageIndex]}</Text>
    </View>
  )
}
