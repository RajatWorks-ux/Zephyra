...
// FIX: animation now fires reliably on 0 → real-value transition by resetting
// the Animated.Value to 0 before each new animation run.
function ScoreCircle({ score }: { score: number }) {
  const anim = useRef(new Animated.Value(0)).current

  useEffect(() => {
    if (score <= 0) {
      // Reset so next transition from 0 → real value animates from the start
      anim.setValue(0)
      return
    }
    // Always reset to 0 first so the sweep animation plays fully each time
    anim.setValue(0)
...
// ─── Timing Context Line ──────────────────────────────────────────────────────
function TimingContextLine({ chartData }: { chartData: ChartData | null }) {
  if (!chartData?.currentTiming) return null
  const { currentAntardasha, userAge } = chartData.currentTiming
  const mahaLord = chartData.vedic.mahadasha.replace(' Mahadasha', '')
  return (
    <Text style={tcl.text}>
      {currentAntardasha.lord} Antardasha in {mahaLord} Mahadasha · Age {userAge}
    </Text>
  )
}

const tcl = StyleSheet.create({
  text: {
    fontFamily: Fonts.accent,
    fontSize: 10,
...
// ─── Sade Sati Warning Card ───────────────────────────────────────────────────
function SadeSatiCard({ endYear, phase }: { endYear: number | null; phase: string | null }) {
  return (
    <View style={ssc.card}>
      <LinearGradient
        colors={['rgba(80,10,10,0.85)', 'rgba(20,5,5,0.9)']}
        style={StyleSheet.absoluteFillObject}
      />
      <View style={ssc.row}>
        <Text style={ssc.icon}>⚠</Text>
        <View style={ssc.textWrap}>
          <Text style={ssc.title}>Sade Sati Active</Text>
          <Text style={ssc.body}>
            Saturn is transiting {phase === 'starting' ? 'into' : phase === 'peak' ? 'through' : 'out of'} your natal Moon sign.
            This 7.5-year cycle brings emotional intensity, patience tests, and karmic clearing.
            {endYear ? ` Easing by ~${endYear}.` : ''}
          </Text>
        </View>
      </View>
    </View>
...
const ssc = StyleSheet.create({
  card: {
    ...
    borderWidth: 1, borderColor: 'rgba(255,80,80,0.3)', position: 'relative',
  },
  row: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 16 },
  icon: { fontSize: 20, color: '#FF4444', marginTop: 1 },
  textWrap: { flex: 1 },
  title: { fontFamily: Fonts.accentBold, fontSize: 13, color: '#FF6666', marginBottom: 4 },
  body: { fontFamily: Fonts.body, fontSize: 12, color: 'rgba(255,255,255,0.6)', lineHeight: 18 },
})

// ─── Jupiter Opportunity Card ─────────────────────────────────────────────────
function JupiterCard({ houseFromMoon }: { houseFromMoon: number }) {
  return (
    <View style={jc.card}>
      <LinearGradient
        colors={['rgba(40,40,5,0.85)', 'rgba(10,10,5,0.9)']}
        style={StyleSheet.absoluteFillObject}
      />
      <View style={jc.row}>
        <Text style={jc.icon}>✦</Text>
        <View style={jc.textWrap}>
          <Text style={jc.title}>Jupiter Favorable Transit</Text>
          <Text style={jc.body}>
            Guru (Jupiter) is transiting House {houseFromMoon} from your natal Moon —
            a fortunate position bringing opportunities for growth, learning, and
            expansion. Act on good ideas that arise now.
          </Text>
        </View>
      </View>
    </View>
...
const jc = StyleSheet.create({
  card: {
    borderRadius: 16, overflow: 'hidden', marginTop: 12,
    borderWidth: 1, borderColor: 'rgba(255,215,0,0.25)', position: 'relative',
  },
  row: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 16 },
  icon: { fontSize: 20, color: '#FFD700', marginTop: 1 },
  textWrap: { flex: 1 },
  title: { fontFamily: Fonts.accentBold, fontSize: 13, color: '#FFD700', marginBottom: 4 },
  body: { fontFamily: Fonts.body, fontSize: 12, color: 'rgba(255,255,255,0.6)', lineHeight: 18 },
})
...
function GeneratingView({
  ...
  return (
    <View style={gen.container}>
      ...
      <Text style={gen.sub}>
        5 AI oracles working simultaneously. Usually ready in about 10 minutes — keep Zephyra open while they work.
      </Text>
    </View>
  )
}
    ...
    <View style={styles.root}>
      ...
      >
          ...
          */}
          {!isWorking && (
            <>
              <View style={styles.heroRow}>
                ...
                <TimingContextLine chartData={chartData} />
                <View style={styles.heroRight}>
                  <View style={styles.pillRow}>
                    <PlanetPill label={`Chandra · ${chartData?.vedic.moonRashi || '—'}`} color="#C9A84C" />
                    <PlanetPill label={`${chartData?.vedic.mahadasha || '—'} Dasha`} color="#7C3AED" />
                  </View>
                  {reading && (
              ...
              {/* [FIX 1] Silent 24-hour refresh indicator — subtle, non-blocking */}
              {silentlyRefreshing && (
                <View style={styles.silentRefreshRow}>
                  <Text style={styles.silentRefreshText}>↻  Refreshing your cosmic data…</Text>
                </View>
              )}
            </>
          )}
          ...
          {hasError && (
            <Text style={styles.errorText}>
              {generationStatus?.includes('backgrounded')
                ? generationStatus
          ...
          <>
            ...
            <View style={styles.guidanceRow}>
              <GuidanceMini
                icon="✦"
                label="Favours"
                value={reading.career_strengths?.[0] || '—'}
                color="#44FF88"
              />
              <View style={styles.guidanceSpacer} />
              {/* BUG #1 FIX: use AI-generated daily_caution; fall back to '—'
                  for older cached readings that don't have this field yet */}
              <GuidanceMini
                icon="△"
                label="Caution"
                value={reading.daily_caution || '—'}
                color="#FF6B6B"
              />
              <View style={styles.guidanceSpacer} />
              {/* BUG #1 FIX: use AI-generated peak_hours; fall back to '—'
                  for older cached readings that don't have this field yet */}
              <GuidanceMini
                icon="◷"
                label="Peak Hours"
                value={reading.peak_hours || '—'}
                color="#C9A84C"
              />
            </View>

            {/* Conditional timing cards */}
            {chartData?.currentTiming?.sadeSatiStatus?.isActive && (
              <SadeSatiCard
                endYear={chartData.currentTiming.sadeSatiStatus.endYear}
                phase={chartData.currentTiming.sadeSatiStatus.phase}
              />
            )}
            {chartData?.currentTiming?.jupiterTransitFavorable && (
              <JupiterCard houseFromMoon={chartData.currentTiming.jupiterHouseFromMoon} />
            )}
          </>
        )}
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
            navigation.navigate('ForecastTab')
          }}
          activeOpacity={0.8}
        >
          <LinearGradient
        ...
        <TouchableOpacity
          style={bondsCardStyle}
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); navigation.navigate('BondsTab') }}
          activeOpacity={0.85}
        >
          <LinearGradient colors={['rgba(47,190,190,0.18)', 'rgba(123,47,190,0.12)']} style={StyleSheet.absoluteFillObject} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} />
          <LinearGradient colors={['#2FBEBE', 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, borderTopLeftRadius: 20, borderTopRightRadius: 20 }} />
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 20 }}>
            <View>
              <Text style={{ fontSize: 22, marginBottom: 6 }}>◍</Text>
              <Text style={{ fontFamily: Fonts.heading, fontSize: 15, color: '#E8E8FF', marginBottom: 3 }}>Relationship Bonds</Text>
              <Text style={{ fontFamily: Fonts.body, fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Vedic compatibility • 8 Kootas • Yogas</Text>
            </View>
            <Text style={{ fontSize: 26, color: 'rgba(47,190,190,0.4)' }}>→</Text>
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // [FIX 1] Subtle indicator shown while 24h background refresh runs
  silentRefreshRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 10,
...
const bondsCardStyle = {
  borderRadius: 20,
  borderWidth: 1,
  borderColor: 'rgba(47,190,190,0.2)',
