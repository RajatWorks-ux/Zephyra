...
import { ApiKeyUpdateModal } from '../../components/settings/ApiKeyUpdateModal'
...
export function ProfileScreen() {
  ...
  // Security layers: 7 taps → biometric (device owner only) → PIN
  // NO device whitelist (removed — recovery impossible if device lost)
  // NO network calls — entirely offline auth
  const handleAvatarTap = useCallback(async () => {
    ...
    try {
      ...
      if (!compatible || !enrolled) {
        // Device has no biometric — silently go straight to PIN
        // (dev device without biometric setup)
        setAdminMode(true)
        return
      }

      // Biometric prompt — hardcoded, cannot be changed via app settings
      const result = await LocalAuthentication.authenticateAsync({
        promptMessage: ADMIN_PROMPT_MESSAGE,
        disableDeviceFallback: false,   // allow PIN/password as device fallback
        cancelLabel: 'Cancel',
      })

      if (result.success) {
        // Biometric passed — launch PIN screen
        setAdminMode(true)
