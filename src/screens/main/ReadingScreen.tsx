...
import { LANGUAGES } from '../../store/settingsStore'
import type { Language } from '../../types'

// ─── TTS Language Code Helper ──────────────────────────────────────────────────
// Preserves region distinctions that matter for TTS (zh-CN vs zh-TW, etc.)
function getTtsLangCode(languageCode: string): string {
  if (languageCode === 'hinglish') return 'hi'
  ...
  if (fullCodeMap[languageCode]) return fullCodeMap[languageCode]
...
function TtsButton({
  ...
  // ── BUG #1 FIX: Mutex prevents double-tap spawning multiple loads ──────────
  const isLoadingRef = useRef(false)
  // ── BUG #1 FIX: Cancel flag — set true when stopped during loading ─────────
  const cancelledRef = useRef(false)

  // When parent sets isPlaying=false (switching chapters), kill audio + cancel load
  useEffect(() => {
    if (!isPlaying) {
      // BUG #1 FIX: mark cancelled so any in-flight createAsync is discarded on resolve
  ...
  const handlePress = useCallback(async () => {
    ...
    if (isPlaying) {
      // Stop: mark cancelled so any in-flight load is discarded when it resolves
      cancelledRef.current = true
      if (soundRef.current) {
    ...
    // ── BUG #1 FIX: If already loading, ignore rapid double-tap entirely ────
    if (isLoadingRef.current) return
    isLoadingRef.current = true
    cancelledRef.current = false

    // Kill any stale sound before starting fresh
    if (soundRef.current) {
      await soundRef.current.stopAsync().catch(() => {})
      await soundRef.current.unloadAsync().catch(() => {})
      soundRef.current = null
    ...
    try {
      onPlay(chapterId)
      // BUG #14 FIX: use helper that preserves zh-CN vs zh-TW distinction
      const langCode = getTtsLangCode(languageCode)
      const encodedText = encodeURIComponent(textToRead)
      const audioUri = `http://127.0.0.1:5000/tts?text=${encodedText}&lang=${langCode}`

      // BUG #11 FIX: configure audio session for both iOS and Android
      ...
      // BUG #1 FIX: Load with shouldPlay:false so we can check cancel flag first
      const { sound } = await Audio.Sound.createAsync(
        { uri: audioUri },
        { shouldPlay: false }
      )

      // ── BUG #1 FIX: If cancelled while network was loading, discard sound ─
      ...
      // BUG #7 FIX: set the completion callback BEFORE calling playAsync.
      // Previously it was set AFTER, so on fast devices or short audio clips
      // didJustFinish could fire before the callback was registered — meaning
      // onStop() was never called and the button stayed stuck as ⏸ forever.
  ...
  return (
    <BlurView intensity={20} tint="dark" style={overlayStyles.card}>
      ...
      <View style={overlayStyles.dotsRow}>
        ...
        <Text style={overlayStyles.dotsLabel}>8 chapters · usually about 10 minutes</Text>
      </View>
    </BlurView>
  )
}
...
function LanguageRegenBanner({ currentLangCode, selectedLang, onRegenerate }: LangBannerProps) {
  ...
  // BUG #5 FIX: dynamically look up the current reading's language name
...
export function ReadingScreen() {
  ...
  // BUG #6 FIX: loadSettings is now called in App.tsx at startup — removed from here
  // BUG #15 FIX: removed the empty pointless useEffect(() => { return () => {} }, [])

  function handleToggle(id: string) {
    LayoutAnimation.easeInEaseOut()
    setVisitedIds((prev) => new Set([...prev, id]))
    setExpandedId((prev) => (prev === id ? null : id))
    // BUG #10 FIX: stop TTS whether switching TO a different chapter OR collapsing the same one
    if (playingChapterId) {
  ...
  function handleLanguageSelect(lang: Language) {
    ...
    // BUG #3 FIX: do NOT call setLanguage here — only save the language AFTER user confirms
    if (!session || !birthProfile) return
    Alert.alert(
      `Regenerate in ${lang.name}?`,
      `Your full reading will be regenerated in ${lang.name}. This takes about 10 minutes — please keep the app open.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Regenerate',
          onPress: () => {
            // BUG #3 FIX: only persist the language selection when user actually confirms
            setLanguage(lang)
            setPlayingChapterId(null)
  ...
  function handleRegenerateInLanguage() {
    ...
    Alert.alert(
      ...
      `Your full reading will be regenerated in ${selectedLanguage.name}. This will take about 10 minutes — please keep the app open.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Regenerate',
    ...
    <View style={styles.root}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.topBarRight}>
          {/* BUG #9 FIX: Refresh button — lets user regenerate without having
              to trigger the language banner mismatch flow */}
          {!isGenerating && reading && (
            <TouchableOpacity
              style={styles.refreshBtn}
              onPress={handleRegenerateInLanguage}
              activeOpacity={0.7}
            >
      ...
      >
        ...
        {/* BUG #12 FIX: Error card shown when generation fails (e.g. Supabase quota exceeded) */}
        {hasError && !isGenerating && (
          <BlurView intensity={15} tint="dark" style={styles.errorCard}>
            <Text style={styles.errorIcon}>⚠</Text>
            <Text style={styles.errorTitle}>Something went wrong</Text>
            <Text style={styles.errorText}>
              {'Your reading could not be generated or saved. This can happen when the server is at capacity. Please try again.'}
            </Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  // BUG #9 FIX: container for refresh + globe so they sit side by side
  topBarRight: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
