...
function MessageBubble({
  message, isStreaming, isThinking, onLongPress,
}: { message: ChatMessage; isStreaming?: boolean; isThinking?: boolean; onLongPress?: () => void }) {
      ...
      <View style={[styles.bubbleRow, isUser ? styles.bubbleRowUser : styles.bubbleRowBot]}>
        ...
        <View style={{ flexShrink: 1, maxWidth: isUser ? '100%' : width * 0.74 }}>
          <BlurView intensity={isUser ? 22 : 16} tint="dark" style={[styles.bubble, isUser ? styles.bubbleUser : styles.bubbleBot, message.failed && styles.bubbleFailed]}>
            ...
            {isThinking ? (
              <Text style={styles.thinkingText}>the oracle is reading your chart...</Text>
            ) : (
...
export function ChatScreen({ route }: any) {
  ...
  // Build display messages including streaming. The placeholder fires the
  // instant isSending flips true — BEFORE streamingText has any content —
  // so there's no dead window where the screen shows nothing while waiting
  // for the first token (which can take real time on a loaded free-tier
  // endpoint). Once the first chunk arrives, this same bubble morphs into
  // the live streaming text via the streamingText branch below.
  const displayMessages: ChatMessage[] = [
    ...messages,
    ...(isSending && !streamingText ? [{
      id: 'thinking', session_id: currentSession?.id ?? '', role: 'assistant' as const,
        ...
        >
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <MessageBubble
                message={item}
                isStreaming={item.id === 'streaming' || item.id === 'thinking'}
                isThinking={item.id === 'thinking'}
                onLongPress={(item.id !== 'streaming' && item.id !== 'thinking') ? () => {
                  Haptics.selectionAsync()
                } : undefined}
              />
            )}
...
const styles = StyleSheet.create({
  ...
  thinkingText: { fontFamily: Fonts.mystical, fontSize: 14, color: Colors.textMuted, fontStyle: 'italic' },
