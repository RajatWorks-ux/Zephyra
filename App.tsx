// App.tsx — PHASE 2
// Added: setupStore.initialize() alongside authStore.initialize()
...
export default function App() {
  ...
  useEffect(() => {
    async function prepare() {
      // All three initialize in parallel — blueprint spec
      await Promise.all([initialize(), loadSettings(), initSetup()])
      if (fontsLoaded) await SplashScreen.hideAsync()
      prefetchAllVideos() // background, no await — downloads R2 videos silently

      try {
        await registerDailyResetTask()
      } catch (e) {
        console.warn('[App] registerDailyResetTask failed (expected on simulator):', e)
      }
    }
    prepare()
  }, [fontsLoaded])
